...
'</div></div>';
    ...
    if ($USER->IsAdmin()) {
        ...
        <script>
            function update(){
                var order = 'update=update&' + $('#items').serialize();
                $.post("/bitrix/components/ampersand/catalog_sortable/update1.php", order, function(data){
                    $('#resp').html(data).fadeIn(1000);
                    setTimeout(function(){
                        $('#resp').fadeOut(1000);}, 1000);
                });
            }

            $(function() {
                $('#resp').hide();
                $('.bx_catalog_list_home').sortable({ forcePlaceholderSize: true, helper: 'clone', placeholder: "bx_catalog_item_container ui-state-highlight", opacity: 0.8, cursor: 'move', update: function(){update();}});
            });
        </script>
        <div id="resp"></div>
    <?}?>
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
    ...
    global $USER;
    if ($USER->IsAdmin()) {
        ?><input type="hidden" value=<? echo $arItem['ID']; ?> name="ID[]" />
        <input type="hidden" value=<?php echo $i; ?> name="SORT[]" />
    <?
    $i++;}?>
